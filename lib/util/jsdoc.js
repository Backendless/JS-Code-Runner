...
class MethodDef {
  ...
  addParam(name, type, optional, description, dictionary, dependsOn) {
    this.params.push(new ParamDef(name, type, optional, description, dictionary, dependsOn))

    if (dictionary || dependsOn) {
      this.argsData[name] = { dictionary, dependsOn }
    }
  }
...
class ParamDef {
  constructor(name, type, optional, description) {
    this.name = name
    this.type = type
...
const tagHandlers = {
  paramdef       : (tag, methodDef, registeredParams) => {
    try {
      ...
      methodDef.addParam(
        paramDef.name,
        TypeDef.fromDoc({ names: [paramDef.type] }),
        !paramDef.required,
      ...
      logger.error(`Error processing paramdef tag: ${ tag.title }`, error)
    }
  },
        ...
        if (item.params) {
          item.params.forEach(param => {
            if (!registeredParams[param.name]) {
              const { name, type, optional,description, dictionary, dependsOn } = param

              methodDef.addParam(name, TypeDef.fromDoc(type), !!optional, description, dictionary, dependsOn)
            }
          })
        }
