...
class MethodDef {
  ...
  addParam({ name, label, type, optional, description, dictionary, dependsOn, uiComponent }) {
    this.params.push(new ParamDef(name, label, type, optional, description))
...
class ParamDef {
  constructor(name, label, type, optional, description) {
    this.name = name
    this.label = label
    this.type = type
        ...
        if (item.params) {
          item.params.forEach(param => {
            if (!registeredParams[param.name]) {
              const { name, label, type, optional, description, dictionary, dependsOn, uiComponent } = param
