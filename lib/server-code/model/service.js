...
class ServiceWrapper {
  constructor(clazz, provider, model) {
    ...
    this.serviceInfo = clazz.serviceInfo
...
const ConfigValueAdapters = {
  [ConfigItems.TYPES.BOOL]: value => typeof value === 'string' ? value === 'true' : value,
  [ConfigItems.TYPES.DATE]: value => typeof value === 'string' ? new Date(Number(value)) : value,
}
...
function buildConfig(declared, actual) {
  ...
  declared.forEach((item) => {
    ...
    const type = ConfigItems.TYPES[item.type]
    const valueAdapter = ConfigValueAdapters[type]

    if (valueAdapter) {
      result[item.name] = valueAdapter(result[item.name])
    }
  })
