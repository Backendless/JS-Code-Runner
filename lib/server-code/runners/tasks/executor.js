...
function initClientSdk(task, opts) {
  if (task.initAppData) {
    ...
    if (opts.automation && opts.automation.internalAddress) {
      Backendless.automationServerURL = opts.automation.internalAddress
    }
...
function enrichTask(task, opts) {
  ...
  if (task.invocationContextDto.deviceType === 'AUTOMATION') {
    delete task.invocationContextDto.httpPath

    if (task.invocationContextDto.httpHeaders) {
      delete task.invocationContextDto.httpHeaders['Raw-Request-URI']
    }
  }
}
