...
const setTerminationHook = runner => {
  async function handleTermination(s) {
    if (!runner || !runner.stopped) {
      logger.info(`Termination signal received. Shutting down.. ${s}`)
...
const startRunner = async params => {
  ...
  try {
    ...
    logger.debugMode = options.debug
    logger.verboseMode = options.verbose
    logger.maxTextLength = options.loggers.maxTextLength
